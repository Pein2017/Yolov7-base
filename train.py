                ...
                if best_fitness == fi:
                    ...
                    if wandb_logger.wandb:
                        wandb_logger.log_model(
                            last.parent, opt, epoch, fi, best_model=True
                        )

                # Optionally delete other checkpoint files, if any exist
                if (wdir / "last.pt").exists():
                    (wdir / "last.pt").unlink()
                for f in wdir.glob("epoch_*.pt"):
                    f.unlink()
                for f in wdir.glob("best_*.pt"):
                    if f != best:  # Don't delete the best model
                        f.unlink()
...
if __name__ == "__main__":
    ...
    # Resume
    wandb_run = check_wandb_resume(opt=opt)
